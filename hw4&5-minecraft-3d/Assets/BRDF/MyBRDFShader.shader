    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float3 Schlick_F(half3 R, half cosA)
            {
                ...
                float3 factor = 1 - cosA;
                float3 factor_2 = factor * factor;
                float3 factor_5 = factor_2 * factor_2 * factor;
                return R + (1 - R) * factor_5;
            }
            ...
            float GGX_D(float roughness, float NdotH)
            {
                ...
                float denuminator = (numerator - 1) * NdotH * NdotH + 1;
                denuminator = denuminator * denuminator * UNITY_PI;
                return numerator / denuminator;
            }

            float CookTorrence_G (float NdotL, float NdotV, float VdotH, float NdotH){
                //// TODO: your implementation
                float g_v = 2 * NdotH * NdotV / VdotH;
                float g_l = 2 * NdotH * NdotL / VdotH;
                return min(1, min(g_v, g_l));
            }
