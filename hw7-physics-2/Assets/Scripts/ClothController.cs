...
public class ClothController : MonoBehaviour
{
    ...
    public void Initialize()
    {
        ...
        clothSimulate.SetFloat("gravity", gravity);
        clothSimulate.SetFloat("drag", drag);
        clothSimulate.SetFloat("dt", stepTime);
        clothSimulate.SetBool("useImplicitEuler", useImplicitEuler);

        //设置碰撞球参数
        clothSimulate.SetVector("spherePos", sphere.transform.localPosition);
        clothSimulate.SetFloat("sphereRadius", sphere.transform.localScale.x / 2);
        clothSimulate.SetFloat("sphereDrag", sphereDrag);

        //计算顶点规模及弹簧原长
        distance = (float)size / (float)(_vertexCountX - 1);
